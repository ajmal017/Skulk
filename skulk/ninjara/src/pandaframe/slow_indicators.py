...
def load_indicators():
    try:
        if len(ninjaraObj.slow_min_pd_DF) >= int(ninjaraObj.parser.get('ma', 'interval')):
            moving_average(int(ninjaraObj.parser.get('ma', 'interval')))
        if len(ninjaraObj.slow_min_pd_DF) >= int(ninjaraObj.parser.get('ema', 'interval')):
            exponential_moving_average(int(ninjaraObj.parser.get('ema', 'interval')))
        if len(ninjaraObj.slow_min_pd_DF) >= int(ninjaraObj.parser.get('macd', 'slow_interval')):
            macd(int(ninjaraObj.parser.get('macd', 'fast_interval')), int(ninjaraObj.parser.get('macd', 'slow_interval')))
        if len(ninjaraObj.slow_min_pd_DF) >= int(ninjaraObj.parser.get('adx', 'interval')):
            average_directional_movement_index(int(ninjaraObj.parser.get('adx', 'interval')),
                                               int(ninjaraObj.parser.get('adx', 'interval_ADX')))
        if len(ninjaraObj.slow_min_pd_DF) >= int(ninjaraObj.parser.get('rsi', 'interval')):
...
def flag_it():
    try:
        # Long Entry
        prev_row = ninjaraObj.slow_min_pd_DF.loc[ninjaraObj.slow_min_pd_DF.index[len(ninjaraObj.slow_min_pd_DF)-2]]
        cur_row = ninjaraObj.slow_min_pd_DF.loc[ninjaraObj.slow_min_pd_DF.index[-1]]
        ...
        if cur_row['EMA'] > prev_row['EMA']\
            ...
            ninjaraObj.long_flags['SL_MAEMA'] = 1
        else:
            ninjaraObj.long_flags['SL_MAEMA'] = 0

        # RSI Flag Settings for long
        if int(ninjaraObj.parser.get('rsi', 'long_low')) <= cur_row['RSI'] <= int(ninjaraObj.parser.get('rsi', 'long_high')):
            ninjaraObj.long_flags['SL_RSI'] = 1
        else:
            ninjaraObj.long_flags['SL_RSI'] = 0
        ...
        if cur_row['PosDI'] > cur_row['NegDI'] and cur_row['ADX'] > prev_row['ADX'] and\
            ...
            ninjaraObj.long_flags['SL_ADX'] = 1
        else:
            ninjaraObj.long_flags['SL_ADX'] = 0
        ...
        if cur_row['MACD'] > cur_row['MACDsign'] \
            ...
            ninjaraObj.long_flags['SL_MACD'] = 1
        else:
            ninjaraObj.long_flags['SL_MACD'] = 0
        ...
        if cur_row['EMA'] < prev_row['EMA'] \
                and cur_row['close'] < cur_row['EMA'] \
                and prev_row['close'] < prev_row['EMA'] and cur_row['MA'] < prev_row['MA']:
            ninjaraObj.short_flags['SL_MAEMA'] = 1
        else:
            ninjaraObj.short_flags['SL_MAEMA'] = 0

        # RSI Flag Settings for shot
        if int(ninjaraObj.parser.get('rsi', 'shot_low')) <= cur_row['RSI'] <= int(ninjaraObj.parser.get('rsi', 'shot_high')):
            ninjaraObj.short_flags['SL_RSI'] = 1
        else:
            ninjaraObj.short_flags['SL_RSI'] = 0
        ...
        if cur_row['NegDI'] > cur_row['PosDI'] and cur_row['ADX'] > prev_row['ADX'] and\
            ...
            ninjaraObj.short_flags['SL_ADX'] = 1
        else:
            ninjaraObj.short_flags['SL_ADX'] = 0

        # MACD Flag Setting for shot
        if cur_row['MACD'] < cur_row['MACDsign']\
            and (cur_row['MACD'] + prev_row['MACD']) < (cur_row['MACDsign'] + prev_row['MACDsign']):
            # and cur_row['MACD'] < float(ninjaraObj.parser.get('macd', 'shot_range')) and \
            ninjaraObj.short_flags['SL_MACD'] = 1
        else:
            ninjaraObj.short_flags['SL_MACD'] = 0
        # print(cur_row.name,str(cur_row['open']),cur_row['high'],cur_row['low'],cur_row['close']
        #       ,cur_row['MA'],cur_row['EMA'],cur_row['MACD'],cur_row['ADX'],cur_row['RSI'])
        # print("*L",ninjaraObj.slow_min_long_flags)
...
def moving_average(n):
    try:
        MA = pd.Series(ninjaraObj.slow_min_pd_DF['close'].tail(n).rolling(n, min_periods=n).mean(), name='MA')
        if 'MA' not in ninjaraObj.slow_min_pd_DF.columns:
            ninjaraObj.slow_min_pd_DF = ninjaraObj.slow_min_pd_DF.join(MA.tail(1))
        else:
...
def exponential_moving_average(n):
    try:
        EMA = pd.Series(ninjaraObj.slow_min_pd_DF['close'].tail(n).ewm(span=n, min_periods=n).mean(), name='EMA')
        if 'EMA' not in ninjaraObj.slow_min_pd_DF.columns:
            ninjaraObj.slow_min_pd_DF = ninjaraObj.slow_min_pd_DF.join(EMA.tail(1))
        else:
...
def macd(n_fast, n_slow):
    try:
        EMAfast = pd.Series(ninjaraObj.slow_min_pd_DF['close'].tail(n_slow+10).ewm(span=n_fast, min_periods=n_slow).mean())
        EMAslow = pd.Series(ninjaraObj.slow_min_pd_DF['close'].tail(n_slow+10).ewm(span=n_slow, min_periods=n_slow).mean())
        ...
        if 'MACD' not in ninjaraObj.slow_min_pd_DF.columns:
            ninjaraObj.slow_min_pd_DF = ninjaraObj.slow_min_pd_DF.join(MACD.tail(1))
        else:
            ninjaraObj.slow_min_pd_DF._set_value(MACD.tail(1).index, 'MACD', MACD.tail(1)[0])

        if 'MACDsign' not in ninjaraObj.slow_min_pd_DF.columns:
            ninjaraObj.slow_min_pd_DF = ninjaraObj.slow_min_pd_DF.join(MACDsign.tail(1))
        else:
            ninjaraObj.slow_min_pd_DF._set_value(MACDsign.tail(1).index, 'MACDsign', MACDsign.tail(1)[0])

        if 'MACDdiff' not in ninjaraObj.slow_min_pd_DF.columns:
            ninjaraObj.slow_min_pd_DF = ninjaraObj.slow_min_pd_DF.join(MACDdiff.tail(1))
        else:
...
def average_directional_movement_index(n, n_ADX):
    try:
        df = ninjaraObj.slow_min_pd_DF.tail(n+10)
        i = 0
        UpI = []
        DoI = []
        ...
        if 'PosDI' not in ninjaraObj.slow_min_pd_DF.columns:
            ninjaraObj.slow_min_pd_DF = ninjaraObj.slow_min_pd_DF.join(PosDI.tail(1))
        else:
            ninjaraObj.slow_min_pd_DF._set_value(ninjaraObj.slow_min_pd_DF.tail(1).index, 'PosDI',
                                            math.ceil(PosDI.loc[PosDI.index[len(PosDI)-2]]*100))

        if 'NegDI' not in ninjaraObj.slow_min_pd_DF.columns:
            ninjaraObj.slow_min_pd_DF = ninjaraObj.slow_min_pd_DF.join(NegDI.tail(1))
        else:
            ninjaraObj.slow_min_pd_DF._set_value(ninjaraObj.slow_min_pd_DF.tail(1).index, 'NegDI',
                                            math.ceil(NegDI.loc[NegDI.index[len(NegDI)-2]]*100))

        if 'ADX' not in ninjaraObj.slow_min_pd_DF.columns:
            ninjaraObj.slow_min_pd_DF = ninjaraObj.slow_min_pd_DF.join(ADX.tail(1))
        else:
            try:
                ninjaraObj.slow_min_pd_DF._set_value(ninjaraObj.slow_min_pd_DF.tail(1).index, 'ADX',
...
def rsi(n):
    try:
        """Calculate Relative Strength Index(RSI) for given data.
            ...
            """
        df = ninjaraObj.slow_min_pd_DF.tail(n)
        i = 0
        UpI = [0]
        DoI = [0]
        for row in df.iterrows():
        ...
        if 'RSI' not in ninjaraObj.slow_min_pd_DF.columns:
            ninjaraObj.slow_min_pd_DF = ninjaraObj.slow_min_pd_DF.join(RSI.tail(1))
        else:
