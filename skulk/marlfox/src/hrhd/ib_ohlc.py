...
class OHLC_history(VukWrapper, VukClient):
    ...
    def historicalDataEnd(self, reqId: int, start: str, end: str):
        ...
        tickbank_path = os.path.join(marlfoxObj.get_with_base_path('common', 'tick_bank'), self.SECTYPE ,str(self.HDATE))
        keys = self.full_day_ohlc[0].keys()
        path = os.path.join(tickbank_path,self.SYMBOL + ".csv")
        ...
        self.disconnect()
        self.gstore.upload_blob(
            marlfoxObj.get_value('common', 'hrhd_bucket'),
            path,
            os.path.join(self.SECTYPE,self.HDATE,self.SYMBOL)
        )
    ...
    def ohlc_data_req_parameter(self, args_symbol, args_date, args_ip="127.0.0.1", args_cid=1, args_port=4002, args_sectype='STK', args_expiry=None,
        ...
        try:
            ...
            if not os.path.exists(os.path.join(marlfoxObj.get_with_base_path('common', 'tick_bank'), self.SECTYPE ,str(self.HDATE))):
                os.makedirs(os.path.join(marlfoxObj.get_with_base_path('common', 'tick_bank'), self.SECTYPE ,str(self.HDATE)), exist_ok=True)
